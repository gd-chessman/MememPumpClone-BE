...
@Injectable()
export class TelegramBossBotService implements OnModuleInit {
    private botToken: string;
    private frontendUrl: string;
    private workerUrl: string;
    private readonly logger = new Logger(TelegramBossBotService.name);
    ...
    constructor(
        ...
        this.botToken = this.configService.get<string>('TELEGRAM_BOT_TOKEN_BOSS', '');
        this.frontendUrl = this.configService.get<string>('URL_BOSS_FRONTEND', '');
        this.workerUrl = this.configService.get<string>('URL_WORKER', 'https://proxy.michosso2025.workers.dev');

        if (!this.botToken) {
            throw new Error('TELEGRAM_BOT_TOKEN_BOSS is missing in .env file');
        }

        if (!this.frontendUrl) {
            throw new Error('URL_BOSS_FRONTEND is missing in .env file');
        }
    }
